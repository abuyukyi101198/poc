...
export function buildPipe(scene, {
  ...
  if (isSector) {
    // Flat annular wall meshes at the two cut faces for depth occlusion.
...
export function updatePipe(pipe, camera) {
  ...
  // Outer cap rings: draw the full sector arc.
  ...
  // Outer silhouettes: omit when the tangent angle falls outside the sector.
  ...
  // Inner cap rings (the hole-opening edges).
