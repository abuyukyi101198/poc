...
export function buildCylinder(scene, {
  ...
  // Main mesh: CylinderGeometry uses (sin,cos) convention; threeArcStart converts.
  ...
  if (isSector) {
    // Flat wall meshes close the sector cuts for correct depth occlusion.
    ...
    // Static edge lines outlining the cut faces.
  ...
  // Dynamic lines — updated every frame by updateCylinder.
...
export function updateCylinder(cyl, camera) {
  ...
  // Cap rings: draw the full sector arc; depthTest + polygonOffset handle occlusion.
  ...
  // Silhouettes: omit when the tangent angle falls outside the sector.
