 ...
 *
 * Contents (updated for the current spec revision — Border Leaf as an
 * ordinary R4 ring member, §4.3/§7.5):
 *   - 1 pod
 ...
 *
 * Ring order (inner -> outer), per spec §3.1:
 *   R1 Pod -> R3 Tier-2/Spine -> R4 Tier-3/Leaf (data, mgmt, and border
 *   leaves, distinguished by `leaf_role`) -> R5 Rack -> R6 Server (outermost)
 *
...
// §3.3 — per-option ring population / labeling / header copy for the tab
// switcher (index.html). Option C's ring names match the global defaults
// (RING_NAMES in render.js), so no overrides are needed here.
export const meta = {
  option: "C",
  label: "Option C",
